...
def test_bijective():
    ...
    networkList = [utils.SimpleMLPreshape([1*8*4, 50, 50, (3*K-1)*1*8*4], [nn.ELU(), nn.ELU(), None], reshapeBack=True, shape=[-1,3 * K - 1, 8, 4]) for _ in range(4)]

    f = flow.NeuralSpline(networkList, maskList, K=K, B=B, prior=p)
    ...
    maskList = torch.cat(maskList, 0).to(torch.float32)
    networkList = [utils.ConvResidualNet(1, 3 * K - 1, 10, 4, activation=F.relu) for _ in range(4)]
    ...
    networkList = [utils.ConvResidualNet(2, 3 * K - 1, 10, 4, activation=F.relu) for _ in range(4)]
    ...
    f = flow.NeuralSpline(networkList, maskList, K=K, B=B, fixFirst=fixFirst, prior=prior)
...
def test_saveload():
    ...
    blankf = flow.NeuralSpline(networkList, maskList, K=K, B=B, prior=p)

    saveload(f, blankf, decimal=4)
