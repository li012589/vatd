...
def test_bijective():
    ...
    networkList = [utils.SimpleMLPreshape([1*8*4, 50, 50, (2 * K - 1)*1*8*4], [nn.ELU(), nn.ELU(), None], reshapeBack=True, shape=[-1,2 * K - 1, 8, 4]) for _ in range(4)]

    f = flow.QuadraticSpline(networkList, maskList, K=K, B=B, prior=p).double()
    ...
    maskList = torch.cat(maskList, 0).to(torch.float32)
    networkList = [utils.ConvResidualNet(1, 2 * K - 1, 10, 4, activation=F.relu) for _ in range(4)]
    ...
    networkList = [utils.ConvResidualNet(2, 2 * K - 1, 10, 4, activation=F.relu) for _ in range(4)]
    ...
    f = flow.QuadraticSpline(networkList, maskList, K=K, B=B, fixFirst=fixFirst, prior=prior).double()
...
def test_saveload():
    ...
    blankf = flow.QuadraticSpline(networkList, maskList, K=K, B=B, prior=p).double()
