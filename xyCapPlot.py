...
def heatCapPlot(model, L, batchSize, target, betaMin=0.5, betaMax=1.5, stepNum=100, savePath=None, show=False, autodiff=True, repAcptRej=False, autodiffLevel=0, device=torch.device('cpu')):
    if savePath is None:
        savePath="./"
    if autodiffLevel == 0:
        savePath += "_level0"
    ...
    for beta in betaLst:
        print(beta.item())
        if repAcptRej:
            beta = beta.detach().repeat(batchSize, 1).requires_grad_(True).to(device)
            T = 1/beta

            z = model.prior.sample(batchSize, T=T)
            if autodiffLevel == 1:
                z = z.detach()
            ...
            dlogP = torch.autograd.grad(logProb, beta, grad_outputs=torch.ones(batchSize, 1).to(device), create_graph=True, retain_graph=True)[0]
            dEdT = torch.autograd.grad(energyWithT, beta, grad_outputs=torch.ones(batchSize, 1).to(device), create_graph=True, retain_graph=True)[0]
            ...
            if autodiffLevel == 0:
                ...
                dlogQdivR = torch.autograd.grad(logQdivR, beta, grad_outputs=torch.ones(batchSize, 1).to(device), create_graph=True)[0]
            ...
            if autodiffLevel == 1:
                dzlogProb = torch.autograd.grad(zlogProb, beta, grad_outputs=torch.ones(batchSize, 1).to(device), create_graph=True, retain_graph=True)[0]
            ...
            z = model.prior.sample(batchSize, T=1/beta)
            z = z.detach()
    ...
    if autodiff or repAcptRej:
        dlogPlst = np.array(dlogPlst)

    lnZerrlst = np.array(lnZerrlst)
    if autodiff:
        deltaMeanlst = np.array(deltaMeanlst)
