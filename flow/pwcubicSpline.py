...
from utils import unconstrained_cubic_spline


class CubicSpline(Flow):
    ...
    def forward(self, x, T=None):
        ...
        for i in range(len(self.networkList)):
            ...
            out = self.networkList[i](upper).reshape(lower.shape[0], lower.shape[1], 2 * self.K + 2, *lower.shape[-2:]).permute([0, 1, 3, 4, 2])

            W, H, D = torch.split(out, self.K, dim = -1)
            DL = D[:, :, :, :, 0][..., None]
            DR = D[:, :, :, :, 1][..., None]

            lower, ld = unconstrained_cubic_spline(
                lower, W, H, DL, DR, inverse=False, tail_bound=self.B)
    ...
    def inverse(self, z, T=None):
        ...
        for i in reversed(range(len(self.networkList))):
            ...
            lower, ld = unconstrained_cubic_spline(
                lower, W, H, DL, DR, inverse=True, tail_bound=self.B)
