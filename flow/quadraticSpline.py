...
from utils import unconstrained_quadratic_spline


class QuadraticSpline(Flow):
    ...
    def forward(self, x, T=None, dilute=2):
        ...
        for i in range(len(self.networkList)):
            ...
            if T is not None:
                _T = torch.zeros(upper.shape[0], 1, upper.shape[-2]//dilute, upper.shape[-1]//dilute, dilute, dilute).to(T)
                if len(T.shape) < 1:
                    T = T.repeat(upper.shape[0])
                _T[:, :, :, :, 0, 0] = T.view([-1] + [1] * (len(upper.shape) - 1)).repeat(1, *_T.shape[1:4])
                _T = _T.permute(0, 1, 2, 4, 3, 5).reshape(upper.shape[0], 1, *upper.shape[2:])
                upper = torch.cat([upper, _T], dim=1)

            out = self.networkList[i](upper).reshape(lower.shape[0], lower.shape[1], 2 * self.K - 1, *lower.shape[-2:]).permute([0, 1, 3, 4, 2])

            W, H = torch.split(out, self.K, dim = -1)

            lower, ld = unconstrained_quadratic_spline(
                lower, W, H, inverse=False, tail_bound=self.B)
    ...
    def inverse(self, z, T=None, dilute=2):
        ...
        for i in reversed(range(len(self.networkList))):
            ...
            lower, ld = unconstrained_quadratic_spline(
                lower, W, H, inverse=True, tail_bound=self.B)
